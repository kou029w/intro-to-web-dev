...
<!-- [💡 NotebookLM で解説を聞く](https://notebooklm.google.com/) -->
...
データがどう流れるのか見ていきましょう。典型的には大きく3つの層に分かれています (フロントエンド・バックエンド・インフラの役割分担に対応していますね)。
...
│                    バックエンド層                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  APIサーバー (Hono、Expressなど)                      │   │
│  │  - リクエストの処理・ビジネスロジックの実行               │   │
│  │  - 認証・認可・セキュリティ管理                          │   │
│  │  - データベースへの読み書き                             │   │
│  │  - レスポンスの生成                                     │   │
│  └─────────────────────────────────────────────────────┘   │
...
│  │  - データの永続化 (ファイルやディスクへの保存)           │   │
│  │  - データの検索・集計・分析                             │   │
│  │  - データの整合性保証 (トランザクション管理)             │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```
...
- **フルスタックフレームワーク** (AstroやNext.jsなど) の登場で、フロントエンドエンジニアもサーバーサイドのコードを書く機会が増えた
- **サーバーレスアーキテクチャ**により、インフラ管理の負担が軽減された
- **小規模チーム**やスタートアップでは、両方を担当することも多い
