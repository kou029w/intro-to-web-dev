...
REST APIを理解することで、Webサービスがどのようにデータをやり取りしているかを把握できるようになります。特に、API設計やフロントエンド・バックエンド間の通信を理解するうえで重要な基礎概念を学びました。

- **REST API**: Web上でデータをやり取りするための代表的な設計原則・方式
- **HTTPメソッド**: GET（取得）、POST（作成）、PUT（更新）、DELETE（削除）といった操作を表す
- **ステータスコード**: リクエストの処理結果を示す3桁の数字（例：200、404）
- **JSON形式**: APIにおける標準的なデータ交換フォーマット
- **リソース指向**: URLは操作ではなく、対象となるリソース（モノ）を表現する
- **ステートレス**: 各リクエストは独立しており、サーバーは過去のやり取りを保持しない

次の記事では、HTTPリクエストの具体的な構造やJSONの詳細について学びます。
開発者ツールを使い、実際のAPI通信を確認しながら理解を深めていきましょう。
